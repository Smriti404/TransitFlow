...
- `make build` - Build the application binary to `bin/TransitFlow`
...
See `loadtest/README.md`. Start with pprof enabled: `TRANSITFLOW_ENABLE_PPROF=1 make run`, then run `k6 run loadtest/k6/scenarios.js`. Capture CPU profiles with `go tool pprof http://localhost:6060/debug/pprof/profile?seconds=30`.
...
TransitFlow supports JSON configuration files with IDE validation via `config.schema.json`:
