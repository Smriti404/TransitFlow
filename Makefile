...
LDFLAGS := -ldflags "-X 'transitflow/internal/buildinfo.CommitHash=$(GIT_COMMIT)' \
                    -X 'transitflow/internal/buildinfo.Branch=$(GIT_BRANCH)' \
                    -X 'transitflow/internal/buildinfo.BuildTime=$(BUILD_TIME)' \
                    -X 'transitflow/internal/buildinfo.Version=$(VERSION)' \
                    -X 'transitflow/internal/buildinfo.CommitTime=$(GIT_COMMIT_TIME)' \
                    -X 'transitflow/internal/buildinfo.Dirty=$(GIT_DIRTY)' \
                    -X 'transitflow/internal/buildinfo.UserEmail=$(GIT_EMAIL)' \
                    -X 'transitflow/internal/buildinfo.UserName=$(GIT_NAME)' \
                    -X 'transitflow/internal/buildinfo.RemoteURL=$(GIT_REMOTE)' \
                    -X 'transitflow/internal/buildinfo.CommitMessage=$(GIT_MSG)' \
                    -X 'transitflow/internal/buildinfo.Host=$(BUILD_HOST)'"
...
build: gtfstidy
	$(SET_ENV) go build -tags "$(BUILD_TAGS)" $(LDFLAGS) -o bin/transitflow ./cmd/api

build-pure: gtfstidy
	CGO_ENABLED=0 go build -tags "$(PURE_TAGS)" $(LDFLAGS) -o bin/transitflow ./cmd/api

build-debug: gtfstidy
	$(SET_ENV) go build -tags "$(BUILD_TAGS)" $(LDFLAGS) -gcflags "all=-N -l" -o bin/transitflow ./cmd/api
...
define run_load_test
	...
	printf '%s\n' \
	  ...
	  '}' > config.ci.json; \
	$(3)./bin/transitflow -f config.ci.json > transitflow.log 2>&1 &
TRANSITFLOW_PID=$$!;
	trap 'kill $$TRANSITFLOW_PID 2>/dev/null || true; rm -f config.ci.json transitflow.log ci-gtfs.db' EXIT; \
	echo "Waiting for TransitFlow to be ready..."; \
	ready=0; \
	for i in $$(seq 1 60); do \
	  ...
	  tail -1 transitflow.log 2>/dev/null || true; \
	  sleep 5; \
	done; \
	if [ "$$ready" -ne 1 ]; then \
...
stresstest: build check-k6
	$(call run_load_test,1000,warn,TRANSITFLOW_ENABLE_PPROF=1 ,-e USE_FALLBACKS=true,loadtest/k6/stress-summary.json)
